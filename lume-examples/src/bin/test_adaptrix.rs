...
struct AdaptrixApp {
    ...
    cluster_children_buffer: Option<lume_vulkan::VulkanBuffer>,
...
impl AdaptrixApp {
    fn new() -> Self {
        ...
        Self {
            ...
            asset: Some(asset), cluster_buffer: None, vertex_buffer: None, vertex_index_buffer: None, primitive_index_buffer: None,
            cluster_children_buffer: None, visible_clusters_buffer: None, visible_count_buffer: None, zero_buffer: None, view_buffer: None,
            ...
            sw_visible_clusters_buffer: None, sw_dispatch_args_buffer: None, sw_zero_dispatch_buffer: None,
            sw_depth_buffer: None, sw_id_buffer: None, clear_sw_pipeline: None, clear_sw_layout: None, clear_sw_bg: None,
            soft_pipeline: None, soft_layout: None, soft_bg0: None, soft_bg1: None, soft_view_proj_buffer: None, soft_viewport_buffer: None,
            hzb_textures: Vec::new(), hzb_views: Vec::new(), hzb_pipeline: None, hzb_layout: None, hzb_bind_groups: Vec::new(),
    ...
    fn setup_gpu_resources(&mut self) {
        ...
        self.cluster_children_buffer = Some(device.create_buffer(BufferDescriptor { size: (asset.cluster_children.len() * 4).max(4) as u64, usage: BufferUsage::STORAGE | BufferUsage::COPY_DST, mapped_at_creation: true }).unwrap());
        if !asset.cluster_children.is_empty() { self.cluster_children_buffer.as_ref().unwrap().write_data(0, bytemuck::cast_slice(asset.cluster_children)).unwrap(); }
        ...
        self.sw_depth_buffer = Some(device.create_buffer(BufferDescriptor { size: pixel_count * 4, usage: BufferUsage::STORAGE | BufferUsage::COPY_DST, mapped_at_creation: true }).unwrap());
        self.sw_id_buffer = Some(device.create_buffer(BufferDescriptor { size: pixel_count * 4, usage: BufferUsage::STORAGE | BufferUsage::COPY_DST, mapped_at_creation: true }).unwrap());
        self.sw_visible_clusters_buffer = Some(device.create_buffer(BufferDescriptor { size: (asset.clusters.len() * 4).min(256 * 1024 * 1024) as u64, usage: BufferUsage::STORAGE, mapped_at_creation: true }).unwrap());
        self.sw_dispatch_args_buffer = Some(device.create_buffer(BufferDescriptor { size: 12, usage: BufferUsage::STORAGE | BufferUsage::COPY_DST | BufferUsage::COPY_SRC | BufferUsage::INDIRECT, mapped_at_creation: true }).unwrap());
        ...
        let zero_buffer = device.create_buffer(BufferDescriptor { size: 16, usage: BufferUsage::COPY_SRC, mapped_at_creation: true }).unwrap();
        ...
        self.vis_depth_texture = Some(device.create_texture(TextureDescriptor { width: size.width, height: size.height, depth: 1, format: TextureFormat::Depth32Float, usage: TextureUsage::DEPTH_STENCIL_ATTACHMENT | TextureUsage::TEXTURE_BINDING }).unwrap());
        ...
        let hzb_tex = device.create_texture(TextureDescriptor { width: size.width, height: size.height, depth: 1, format: TextureFormat::R32Float, usage: TextureUsage::STORAGE_BINDING | TextureUsage::TEXTURE_BINDING }).unwrap();
        let hzb_view = device.create_texture_view(&hzb_tex, TextureViewDescriptor { format: None }).unwrap();
        self.hzb_textures.push(hzb_tex); self.hzb_views.push(hzb_view);
        ...
        let bgl_c1 = device.create_bind_group_layout(BindGroupLayoutDescriptor { entries: vec![
            BindGroupLayoutEntry { binding: 0, visibility: ShaderStage::COMPUTE, ty: BindingType::UniformBuffer },
            BindGroupLayoutEntry { binding: 1, visibility: ShaderStage::COMPUTE, ty: BindingType::SampledTexture },
        ] }).unwrap();
        let l_cull = device.create_pipeline_layout(PipelineLayoutDescriptor { bind_group_layouts: &[&bgl_c0, &bgl_c1], push_constant_ranges: &[] }).unwrap();
        ...
        self.cull_bind_group_0 = Some(device.create_bind_group(BindGroupDescriptor { layout: &bgl_c0, entries: vec![
            ...
            BindGroupEntry { binding: 6, resource: BindingResource::Buffer(self.cluster_children_buffer.as_ref().unwrap()) },
        ] }).unwrap());
        self.cull_bind_group_1 = Some(device.create_bind_group(BindGroupDescriptor { layout: &bgl_c1, entries: vec![
        ...
        let l_clear = device.create_pipeline_layout(PipelineLayoutDescriptor { bind_group_layouts: &[&bgl_clear], push_constant_ranges: &[PushConstantRange { stages: ShaderStage::COMPUTE, offset: 0, size: 4 }] }).unwrap();
        ...
        self.clear_sw_bg = Some(device.create_bind_group(BindGroupDescriptor { layout: &bgl_clear, entries: vec![
        ...
        let bgl_s0 = device.create_bind_group_layout(BindGroupLayoutDescriptor { entries: vec![
            ...
            BindGroupLayoutEntry { binding: 2, visibility: ShaderStage::COMPUTE, ty: BindingType::StorageBuffer },
            BindGroupLayoutEntry { binding: 3, visibility: ShaderStage::COMPUTE, ty: BindingType::StorageBuffer },
            BindGroupLayoutEntry { binding: 4, visibility: ShaderStage::COMPUTE, ty: BindingType::StorageBuffer },
            BindGroupLayoutEntry { binding: 5, visibility: ShaderStage::COMPUTE, ty: BindingType::StorageBuffer },
        ] }).unwrap();
        let bgl_s1 = device.create_bind_group_layout(BindGroupLayoutDescriptor { entries: vec![
            BindGroupLayoutEntry { binding: 0, visibility: ShaderStage::COMPUTE, ty: BindingType::StorageBuffer },
            BindGroupLayoutEntry { binding: 1, visibility: ShaderStage::COMPUTE, ty: BindingType::StorageBuffer },
            BindGroupLayoutEntry { binding: 2, visibility: ShaderStage::COMPUTE, ty: BindingType::UniformBuffer },
            BindGroupLayoutEntry { binding: 3, visibility: ShaderStage::COMPUTE, ty: BindingType::UniformBuffer },
        ] }).unwrap();
        let l_soft = device.create_pipeline_layout(PipelineLayoutDescriptor { bind_group_layouts: &[&bgl_s0, &bgl_s1], push_constant_ranges: &[] }).unwrap();
        ...
        let l_vis = device.create_pipeline_layout(PipelineLayoutDescriptor { bind_group_layouts: &[&bgl_v0, &bgl_v1], push_constant_ranges: &[] }).unwrap();
        ...
        self.vis_render_pass = Some(vis_rp); self.vis_layout = Some(l_vis);
        ...
        let l_res = device.create_pipeline_layout(PipelineLayoutDescriptor { bind_group_layouts: &[&bgl_r0, &bgl_r1], push_constant_ranges: &[] }).unwrap();
        ...
        self.resolve_render_pass = Some(res_rp); self.resolve_layout = Some(l_res);
...
impl ApplicationHandler for AdaptrixApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.window.is_none() {
            let window = Arc::new(event_loop.create_window(Window::default_attributes().with_title("LumeVisual - Nanite High Perf").with_inner_size(winit::dpi::LogicalSize::new(1280.0, 720.0))).unwrap());
            ...
            WindowEvent::RedrawRequested => {
                if let (Some(device), Some(swapchain)) = (&self.device, self.swapchain.as_mut()) {
                    ...
                    self.view_buffer.as_ref().unwrap().write_data(0, bytemuck::bytes_of(&ViewUniform {
                        ...
                        inv_view_proj: [vp.inverse().col(0), vp.inverse().col(1), vp.inverse().col(2), vp.inverse().col(3)],
                        camera_pos_and_threshold: glam::vec4(cam_pos.x, cam_pos.y, cam_pos.z, 1.0), 
                        viewport_size: glam::vec4(1280.0, 720.0, 0.0, 0.0),
                    })).unwrap();
                    if let Some(buf) = self.soft_view_proj_buffer.as_ref() { buf.write_data(0, bytemuck::cast_slice(&[vp.col(0), vp.col(1), vp.col(2), vp.col(3)])).unwrap(); }
                    if let Some(buf) = self.soft_viewport_buffer.as_ref() { buf.write_data(0, bytemuck::cast_slice(&[0.0f32, 0.0f32, 1280.0f32, 720.0f32])).unwrap(); }
